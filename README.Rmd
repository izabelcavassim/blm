...
**```make_prior()``` Constructs a prior distribution for the weight vectors, which are Normally distributed: $\mathrm{N}(\mathbf{0},\,\mathbf{S}_0)$ with
...
* ```confint()``` Returns the lower and upper confidence intervals of a parameter (parm) given. The usage of confint is as follow: ```confint(object, parm, level = 0.95, ...)```. The parameter parm can be assigned with numbers (columns of the model data.frame) or names: 'y', 'x', 'z'...
